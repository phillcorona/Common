...
char	*get_next_line(int fd)
{
	...
	if (fd < 0 || BUFFER_SIZE <= 0 || fd >= FD_MAX) // Validate input
		return (NULL);
	buffer[fd] = ft_read_to_buffer(fd, buffer[fd]); // Read data into the buffer
	if (!buffer[fd])
		return (NULL);
	line = ft_extract_line(&buffer[fd]); // Extract a single line from the buffer
	if (!buffer[fd][0]) // Free buffer if empty
	{
		free(buffer[fd]);
		buffer[fd] = NULL;
	}
